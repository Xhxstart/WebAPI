...
namespace Codeplex.Data
{
    public class DynamicJson : DynamicObject
    {
        ...
        private object DeserializeObject(Type targetType)
        {
            ...
            {
                if ("report_data".Equals(item.Name.ToString()))
                {
                    var dic = new Dictionary<string, object>();
                    item.Elements().ToList().ForEach(x =>
                    {
                        var name = x.Name.LocalName;
                        var it = x.Attribute("item");
                        if ((name == "item") && (it != null))
                        {
                            name = it.Value;
                        }
                        dic.Add(name, ToValue(x));
                    });
                    PropertyInfo propertyInfo;
                    if (!dict.TryGetValue(item.Name.LocalName, out propertyInfo)) continue;
                    propertyInfo.SetValue(result, (dynamic)dic, null);
                }
                else
                {
                    PropertyInfo propertyInfo;
